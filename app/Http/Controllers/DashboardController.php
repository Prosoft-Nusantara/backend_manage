...
class DashboardController extends Controller
{
    public function highlightProject()
    {
        // Total semua proyek
        $projectCount = [
            'jumlah_proyek' => Project::where('status', '!=', '3')->count(),
            'total_nilai_kontrak' => Project::where('status', '!=', '3')->sum('total_nilai_kontrak'),
            'realisasi_budget' => Project::where('status', '!=', '3')->sum('realisasi_budget'),
            'margin' => Project::where('status', '!=', '3')->sum('total_nilai_kontrak') - Project::where('status', '!=', '3')->sum('realisasi_budget')
        ];
